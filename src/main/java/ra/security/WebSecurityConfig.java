...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.cors() // ngăn chặn request từ một domain khác
                .and().csrf().disable()
                .authorizeRequests()
                .antMatchers("/api/auth/**").permitAll() //cho phép tất cả mọi người truy cập vào địa chỉ nào
                .antMatchers("/api/**").permitAll()
                .antMatchers("/api/blogs").permitAll()
                .antMatchers("/api/admin").hasRole("ADMIN")
                .antMatchers("/api/catalogs").hasRole("ADMIN")
                .anyRequest().authenticated() // tất cả các request cần phải xác thực
                .and()
                .logout().logoutRequestMatcher(new AntPathRequestMatcher("/api/auth/logout")).logoutSuccessUrl("/api/auth/signin");
